...
func (client *Client) Request(method, path string, pathParams, queryParams urlParams, body ...dtos.DTO) (resBody io.ReadCloser, err error) {
	req, err := buildRequest(method, path, pathParams, queryParams, body...)
	if err != nil {
		return
	}
...
func buildRequest(method, path string, pathParams, queryParams urlParams, bodies ...dtos.DTO) (req *http.Request, err error) {
	path, err = pathRender(path, pathParams)
	if err != nil {
		return
	}
	if len(bodies) > 0 {
		req, err = buildBodyRequest(method, path, bodies[0])
	} else {
		req, err = buildBodilessRequest(method, path)
	}
	return
}
