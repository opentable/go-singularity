...
func (self *OfferID) SetField(name string, value interface{}) error {
	...
	switch name {
	default:
		return fmt.Errorf("No such field %s on OfferID", name)
...
func (self *OfferID) GetField(name string) (interface{}, error) {
	switch name {
	default:
		return nil, fmt.Errorf("No such field %s on OfferID", name)
...
func (self *OfferID) ClearField(name string) error {
	if self.present == nil {
		self.present = make(map[string]bool)
	}
	switch name {
