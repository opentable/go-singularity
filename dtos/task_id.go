...
func (self *TaskID) SetField(name string, value interface{}) error {
	...
	switch name {
	default:
		return fmt.Errorf("No such field %s on TaskID", name)
	...
	case "defaultInstanceForType", "DefaultInstanceForType":
		...
		if ok {
			...
			return fmt.Errorf("Field defaultInstanceForType/DefaultInstanceForType: value %v couldn't be cast to type *TaskID", value)
		}
...
func (self *TaskID) GetField(name string) (interface{}, error) {
	switch name {
	default:
		return nil, fmt.Errorf("No such field %s on TaskID", name)
...
func (self *TaskID) ClearField(name string) error {
	if self.present == nil {
		self.present = make(map[string]bool)
	}
	switch name {
