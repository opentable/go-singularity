...
func (self *SlaveID) MarshalJSON() ([]byte, error) {
...
func (self *SlaveID) FieldsPresent() []string {
	return presenceFromMap(self.present)
}

func (self *SlaveID) SetField(name string, value interface{}) error {
	if self.present == nil {
		self.present = make(map[string]bool)
	}
	switch name {
	default:
		return fmt.Errorf("No such field %s on SlaveID", name)
	...
	case "defaultInstanceForType", "DefaultInstanceForType":
		...
		if ok {
			...
			return fmt.Errorf("Field defaultInstanceForType/DefaultInstanceForType: value %v couldn't be cast to type *SlaveID", value)
		}
...
func (self *SlaveID) GetField(name string) (interface{}, error) {
	switch name {
	default:
		return nil, fmt.Errorf("No such field %s on SlaveID", name)
...
func (self *SlaveID) ClearField(name string) error {
	if self.present == nil {
		self.present = make(map[string]bool)
	}
	switch name {
...
func (self *SlaveID) LoadMap(from map[string]interface{}) error {
